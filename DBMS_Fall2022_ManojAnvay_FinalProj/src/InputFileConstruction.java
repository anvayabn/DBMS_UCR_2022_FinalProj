...
//                    For the Line is valid: there should be a state value, the country should be US and should not be the header
                if (parts[3].equals("US") && parts[4].length()>0 && !(parts[0].equals("USAF")))
                {
                    newKey.set(parts[0]);
                    String statemap_values = String.format("%s,%s", parts[4], parts[0]);
                    newValue.set("W " + statemap_values);
    ...
    public static class Map2 extends Mapper <Object, Text, Text, Text>
    {
                ...
                throws IOException, InterruptedException
        {
            ...
            if (!(line.length() == 0) && !(line.substring(0, 6).equals("STN---"))) {
                ...
                String stationmap_values = String.format("%s,%.2f,%.2f", date, average_temperature, number_of_reading);
                newValue.set("S " + stationmap_values);
                context.write(newKey,newValue);
            }
        }
    }
    ...
    {
        private ArrayList<Text> weatherList = new ArrayList<Text>();
        private ArrayList<Text> stationList = new ArrayList<Text>();
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = new Job(conf, "Input_File_Contruction");
